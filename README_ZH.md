...
- 自动结算：合约按规则发放奖励或扣罚
- 审计追踪：全流程可回放、可验证
- 核心增强：**DR 证明协议（DR Attestation Protocol）** 将链下遥测转为链上可验证证据
  （`telemetry -> baseline 推理 -> 异常门卫 -> 多方证明 -> proof hash 上链`）
