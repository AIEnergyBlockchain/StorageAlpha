...
/**
 * Chain action script for Custom L1 (DR-L1).
 *
 * Identical logic to fuji_chain_action.js but reads RPC URL, chain ID,
 * and deployment report from DR_L1_* env vars instead of Fuji defaults.
 *
 * Env vars:
 *   DR_L1_RPC_URL       — RPC endpoint for the Custom L1 (required)
 *   DR_L1_CHAIN_ID      — Chain ID (informational, for logging)
 *   DR_L1_DEPLOY_OUT    — Path to deployment report JSON (default: cache/l1-deployment-latest.json)
 *   PRIVATE_KEY          — Deployer/operator private key (required for tx actions)
...
const DEFAULT_L1_DEPLOY_OUT = "cache/l1-deployment-latest.json";
...
function requirePrivateKey() {
  ...
  if (!privateKey) {
    throw new Error("PRIVATE_KEY is required for Custom L1 chain mode");
  }
  return privateKey;
}

function resolveDeployment() {
  const deployOut = process.env.DR_L1_DEPLOY_OUT || DEFAULT_L1_DEPLOY_OUT;
  const deployPath = path.resolve(process.cwd(), deployOut);
  if (!fs.existsSync(deployPath)) {
    throw new Error(`L1 deployment report not found: ${deployOut}`);
  }
  const deployment = readJson(deployPath);
  const contracts = deployment.contracts || {};
  if (!contracts.event_manager || !contracts.proof_registry || !contracts.settlement) {
    throw new Error(
      "L1 deployment report missing contracts.event_manager/proof_registry/settlement"
    );
  }
...
async function main() {
  ...
  if (!action) {
    throw new Error(
      "Missing action. Expected: create_event | close_event | submit_proof | settle_event | claim_reward | check_tx"
    );
  }
  ...
  const rpcUrl = process.env.DR_L1_RPC_URL;
  if (!rpcUrl) {
    throw new Error("DR_L1_RPC_URL is required for Custom L1 chain action");
  }

  const provider = new ethers.JsonRpcProvider(rpcUrl);
  const network = await provider.getNetwork();
  const chainName = `dr-l1:${network.chainId.toString()}`;
  ...
  if (action === "create_event") {
    txInfo = await sendManagedTx(
      ...
      provider, wallet, chainName, confirmMode
    );
